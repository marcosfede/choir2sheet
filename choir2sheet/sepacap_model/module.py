...
class Separator(torch.nn.Module):
    def __init__(self, num_stages: int, relative_positional_encoding: dict, enc_stage: dict, spk_split_stage: dict, simple_fusion:dict, dec_stage: dict):
        ...
        class SepEncStage(torch.nn.Module):
            def __init__(self, global_blocks: dict, local_blocks: dict, down_conv_layer: dict, down_conv=True, num_patterns=2):
                super().__init__()
                ...
                for _ in range(num_patterns):
                    self.g_blocks.append(GlobalBlock(**global_blocks))
                    self.l_blocks.append(LocalBlock(**local_blocks))
            ...
            def forward(self, x: torch.Tensor, pos_k: torch.Tensor):
                ...
                for g_block, l_block in zip(self.g_blocks, self.l_blocks):
                    x = g_block(x, pos_k)
        ...
        class SepDecStage(torch.nn.Module):
            def __init__(self, num_spks: int, global_blocks: dict, local_blocks: dict, spk_attention: dict, num_patterns: int=3):
                super().__init__()
                

                self.g_blocks = nn.ModuleList()
                self.l_blocks = nn.ModuleList()
                self.spk_attn = nn.ModuleList()
                
                for _ in range(num_patterns):
                    self.g_blocks.append(GlobalBlock(**global_blocks))
                    self.l_blocks.append(LocalBlock(**local_blocks))
                    self.spk_attn.append(SpkAttention(**spk_attention))
            ...
            def forward(self, x: torch.Tensor, pos_k: torch.Tensor):
                ...
                x: [B, N, T]
                '''
                for g_block, l_block, spk_attn in zip(self.g_blocks, self.l_blocks, self.spk_attn):
                    x = g_block(x, pos_k)
                    x = x.permute(0, 2, 1).contiguous()
                    x = l_block(x)
                    x = x.permute(0, 2, 1).contiguous()
                    x = spk_attn(x, self.num_spk)
        ...
        for _ in range(self.num_stages):
            self.upconv.append(torch.nn.ConvTranspose1d(in_channels=simple_fusion['out_channels'],out_channels=simple_fusion['out_channels'], kernel_size=2, stride=2))
...
class OutputLayer(torch.nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_spks: int, masking: bool = False):
        ...
        self.spe_block = Masking(in_channels, Activation_mask="Sigmoid", concat_opt=None)
        self.num_spks = num_spks
