...
class Model(torch.nn.Module):
    ...
    def peak_normalize_spkbat(self, x: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
        """
        Peak-normalize audio shaped (S, B, T) so that each (spk, batch) slice
        has max(abs) == 1.0.
        """
        # x.abs().amax over time axis → shape (S, B)
        peak = x.abs().amax(dim=2, keepdim=True)    # (S, B, 1)
        return x / (peak + eps)

    def forward(self, x):
        encoder_output = self.audio_encoder(x)
        projected_feature = self.feature_projector(encoder_output) # torch.Size([B, Channels, 3997])
        ...
        audio = [audio[idx] for idx in range(self.num_spks)]
